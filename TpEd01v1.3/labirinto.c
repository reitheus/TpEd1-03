...
Labirinto* lelabirintoInterface(const char str[TAM]){
    ...
    fscanf(file, "%d %d", &L, &C);// leitura das dimenções do labirinto
    printf("%i %i", L, C);
    fscanf(file, "%c", &lixo);//eliminação do \n
    fscanf(file, "%c", &opcao);//leitura da opção de impressão
    ...
    for (int i = 0; i < L; i++)
    {
        for (int j = 0; j < C; j++)
        {
            //fscanf(file, "%c", &(plab->mapa[i][j]));
            plab->mapa[i][j] = fgetc(file);
        }
    }
    //printf("\nbatata\n");
